...
**음성대조가 핵심인 이유.** 폴드 성공 개수만으로 판정하면 순수 노이즈도 "6폴드 중 5폴드 양수"를 약 46% 확률로 통과한다. 따라서 진짜 판별 기준은 폴드 수가 아니라 "전략이 자기 자신의 랜덤 버전을 유의하게 이기는가"이다. 정석 개념은 문헌을 따른다(§8).
...
| 축 | 볼록형 (트레이더 엣지) | 확산형 (기관 엣지) |
|---|---|---|
| 대표 사례 | 급등주 스윙(개미 투매 역발상) | PEAD(실적 드리프트) |
| 승률 | 낮음 (≈32%) | 높음 (≈63%) |
| 분포 왜도 | 높음 (+5.9) | 낮음 |
| 상위 5건의 P&L 점유 | 39–67% | ≈3% |
| 엣지의 출처 | 소수 대박 꼬리 | 수많은 얇은 엣지의 평균 |
| walk-forward 재현성 | 깨지기 쉬움 | 안정적 |

볼록형은 손절로 왼꼬리를 자르고 승자를 태워 오른꼬리를 사지만, 수익이 소수 사건에 집중될수록 재현이 불안정하다. 확산형은 개별 트레이드가 얇은 대신 분산으로 변동성을 낮춰 안정적 Sharpe를 만든다 — 이것이 PEAD가 트레이더 엣지가 아니라 **기관 엣지**인 이유다.

### 3.2 진입 시점 신호의 선험적 예측력

개별 트레이드 분포를 진입 시점 특성으로 층화하면 구조적 규칙성이 드러난다. 급등주 스윙에서 **진입 시점의 모멘텀 강도**로 5분위를 나누면, 사후 트레이드의 기대 R-멀티플이 Q1에서 Q5로 **단조 증가**하며 이 단조성이 TRAIN과 OOS에서 **모두** 유지된다(꼬리(≥+3R) 확률도 하위 분위 대비 상위 분위에서 수 배). 즉 "어느 트레이드가 대박이 될지"는 순전한 운이 아니라 진입 시점에 부분적으로 예측 가능하다. 다만 이 예측력만으로는 집중·비용을 반영한 배포 기준을 넘지 못한다 — 신호가 정보를 담는 것과 그 정보가 거래비용을 이기는 것은 별개다. 분석 코드: [`research/experiments/pead_gate.py`](research/experiments/pead_gate.py), 게이트 하버스: `prop_gate`.
...
| 가설 | 유형 | 관측된 메커니즘 | 기록 |
|---|---|---|---|
| 개미 투매 역발상 | 볼록·급등주 스윙 | 수익이 상위 1%(P&L의 84%)에 집중 → 단일 국면 의존, walk-forward 2/6 | [logs](research/logs/) |
