...
| 종목[수] | 그 (시장,섹터)의 상장 종목 수. 10개 미만은 `~` 마커와 회색 — 사실상 단일종목인 라벨이 있다(부동산 3, 출판/매체복제 2) |
| G[0~1] | 성장 점수. 힘(가속)·압축(미실현)·풀림(ẍ) 세 순위의 평균. `*` 는 세 조건(a>0·x>0·ẍ>0)을 다 만족. **검증된 적 없는 탐색 지표다** |
| 임펄스[억] | 구간 누적 순매수 = Σ(순매매 수량 × 그날 종가) |
| 가속[%p] | 임펄스 ÷ 구간말 섹터 시총 × 100. 물리로 a = F/m |
| 1년[%ile] | 그 주체의 롤링 N일 순매수 합이 **최근 1년 분포에서 몇 등**인가. 100=1년 최대 매수. 이 표에서 **유일한 시계열 맥락**이다 |
...
| 예상Δv | k × 가속 + b. k·b 는 그 구간 27개 섹터의 횡단면 OLS |
| 미실현[%p] | 예상Δv − 실제 수익률. `+` 면 덜 갔고(눌림), `−` 면 이미 더 갔다. 27개 합이 0 인 **상대** 지표다 |
| 포텐셜[½kx²] | ½·k·x². k 가 블록당 상수라 **\|미실현\| 의 순증가 변환**이다 — 정보가 겹친다 |
| dW/dt · 풀림 | 구간을 갈라 본 W=가속×수익률 의 변화, 미실현이 해소되는 가속(ẍ) |
| 순매수/순매도상위 | 그 섹터에서 가장 많이 산/판 종목과 금액. 표는 1개씩, 하단 패널은 3개씩 |
