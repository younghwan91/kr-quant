...
섹터 × 4개 구간(5·20·60·120거래일) × 3개 시장(전체·거래소·코스닥) × 4개 주체(기관·
외국인·개인·기타법인)를 키 하나로 갈아 끼운다.

<!-- SCREENSHOT: 기본 20일 화면 -->
![기본 20일 화면. 섹터 이름 왼쪽부터 종목 수·G·임펄스·가속·1년 백분위·누적 추이 막대·구간 수익률·미실현·풀림, 오른쪽 끝에 그 섹터에서 가장 많이 산 종목과 금액. 가속 내림차순으로 정렬돼 있다.](docs/images/kqflow-table.png)

*기본 화면(20거래일·전체 시장·기관). 한 행이 한 섹터이고, 가속 내림차순이다. 아래
패널은 선택한 섹터의 순매수·순매도 상위 종목을 보여준다.*
...
<!-- SCREENSHOT: Enter 드릴다운 종목 목록 -->
![드릴다운 종목 목록. 종목명·코드·순매수 금액·누적 기여율·참여율·시총대비·시총·거래대금
여덟 열이 순매수 절댓값 순으로 늘어서 있고, 누적 80% 지점에 마커가 찍혀 있다.](docs/images/kqflow-names.png)

*`Enter` 드릴다운. 누적[%] 열의 `-` 마커 위쪽이 그 섹터를 실제로 움직인 종목이다.*
...
한영 상태에서도 위 키가 그대로 듣는다. 다만 자판이 Shift 를 구분하지 않는 자리가 있어
**대문자 역방향은 `W`·`R` 만** 된다(끝으로 가려면 End).
...
*`?` 도움말. 86줄을 스크롤하며 읽는다 — 열의 뜻과 그 열의 한계가 같은 자리에 있다.*

### 3.2 섹터 표의 열

화면에 나오는 **왼쪽부터의 순서**다. 폭이 모자라면 오른쪽 열부터 통째로 빠지므로,
좁은 터미널에서는 아래 표의 뒤쪽이 없다. 실측한 등장 문턱(칸): 수익률 68 · 미실현 79 ·
풀림 92 · G 100 · 순매수상위 124 · 순매도상위 148 · 포텐셜 161 · dW/dt 174 · 종목[수] 183.

| 열 | 뜻 |
|---|---|
| 섹터 | 벤더 분류(`stocks.sector`) 27개. **KRX 업종 분류와 다르다** |
| (마커) | 섹터 이름 오른쪽의 `~` 는 상장 종목 10개 미만, `*` 는 세 조건(a>0·x>0·ẍ>0)을 다 만족. `~` 행은 회색으로 죽인다 — 벤더 분류가 좁아 사실상 단일종목인 라벨이 있다(부동산 3, 출판/매체복제 2) |
...
| 포텐셜[½kx²] | ½·k·x². k 가 블록당 상수라 **\|미실현\| 의 순증가 변환**이다(4개 구간 전부 Spearman 1.0000) — 정보가 겹쳐서 넓은 폭에서만 나온다 |
| dW/dt[%p/일] | 구간을 반으로 갈라 본 W=가속×수익률 의 변화. 힘과 운동이 정렬되는가 |
| 종목[수] | 그 (시장,섹터)의 상장 종목 수. 오른쪽 끝 열이다 |
...
- **4주체 순매수** — `개인 -116 · 외국인 -80 · 기관 +214 · 기타법인 -21 [억]`. 화면은
  ...
  주체도 여기 같이 들어간다(위 표에서 보던 그 값이다). 잔여(= −Σ4주체)는 여기 안 적고
  `kq-ledger` 가 별도 열로 맡는다. 종합 구간에서는 줄 끝에 `· 20일 기준` 이 붙는다.
...
| 열 | 뜻 |
|---|---|
| 순매수[억] | 그 구간 **선택된 주체**의 순매수. 기본 정렬은 이것의 **절댓값** 순 — 많이 산 종목과 많이 판 종목이 같이 위로 온다 |
| 누적[%] | \|순매수\| 큰 순으로 훑을 때의 누적 기여율. `-` 마커가 80% 지점이고, 그 위가 이 섹터를 움직인 종목이다 |
| 참여율[%] | 순매수 ÷ 그 종목 거래대금 × 100. "그 거래의 몇 %가 한 방향이었나" — 분모가 유동성이라 **살 수 있는 종목인가**를 같이 말한다 |
| 시총대비[%p] | 순매수 ÷ 그 종목 시총 × 100. 시총 작은 스팩이 위로 올라온다 |
| 시총 · 거래대금[억] | 위 두 열의 분모 |

⚠️ **`종합` 에서 `Enter` 로 들어간 종목 목록은 20일 기준이다.** 종합은 세 창을 섞은
축이라 "이 종목이 얼마를 샀나" 가 한 값으로 정의되지 않는다. 그래서 드릴다운은 20일
창의 금액을 쓴다(`flow_view.py` 의 `win = self.window if self.window != "종합" else "20"`).
60·120일 금액을 보려면 `w` 로 그 구간에 들어간 뒤 `Enter` 를 눌러라. 종합 화면의 상세
패널도 같은 이유로 4주체 줄 끝에 `· 20일 기준` 을 달고 있다.
...
<!-- SCREENSHOT: 종합 구간 -->
![종합 구간 화면. 섹터마다 종합 G 와 20일·60일·120일 각각의 G, 세 조건을 만족한 창의
수(통과[구간]), 종목 수만 있는 좁은 표.](docs/images/kqflow-combined.png)
...
화면도 그렇게 말한다(방향 화살표를 아예 안 그린다). 창마다 정렬 축이 달라지면 "세 창을
나란히 본다" 는 이 화면의 용도가 무너지기 때문이다. 금액·수익률로 줄을 세우려면 `w` 로
개별 구간에 들어가라.
...
첫 번째는 원론에 더해 **실측이 전제를 부정한다.** 섹터쌍 순매수 상관에서 음수 쌍의 비율:
...
그 셋에 대해 없는 현상을 그리는 셈이다. 기타법인 0.47 은 널 0.50 과 구분되지 않아 어느
방향으로도 말할 것이 없다.
...
- **10종목 미만 섹터는 섹터로 읽지 않는다.** 벤더 분류가 좁아 사실상 단일종목인 라벨이 있다.
