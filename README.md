...
- **생존편향.** 시세·실적은 상장폐지 종목까지 복원했고(460·364종목), 유니버스 무결성은 `storage.read_prices`가 로딩 시점에 검사한다. **남은 구멍은 `shares_outstanding_history`·`supply_demand`** — 폐지분이 비어 있어 시총 기반 유니버스는 여전히 생존자만 담는다. 상장주식수는 DART `stockTotqySttus`로 메울 경로를 확인했고(표본 92%, PIT 구성 가능), 수급은 아직 경로가 없다. 폐지 손실 실현은 `delisting_exit=True`로 선택 가능하며 실측 영향은 미미했다(정리매매 구간이 이미 가격에 들어 있어서).
