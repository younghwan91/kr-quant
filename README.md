...
표 아래 패널은 선택한 섹터를 푼다 — **4주체 순매수 한 줄**(개인·외국인·기관·기타법인,
합이 0 이 되는 회계 항등식이다), 순매수·순매도 상위 3개씩, 그리고 지금 정렬한 열이
무슨 뜻인지 한 줄.
...
60·120일 금액을 보려면 `w` 로 그 구간에 들어간 뒤 `Enter` 를 눌러라.

### 3.4 `종합` 구간

`w` 로 끝까지 돌리면 나오는 `종합` 은 **20·60·120일 세 창의 G 순위를 등가중으로 섞은**
화면이다. 5일이 빠지는 이유는 규칙이 아니라 산술이다 — ẍ 가 2차 차분이라 최소 9거래일을
요구해서 5일 창에는 G 가 아예 없다.
