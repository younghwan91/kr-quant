...
> (`view_width`, curses 가 오른쪽 아래 칸에 쓰면 스크롤한다), 뷰 폭으로 잰 값을
> 터미널 폭인 것처럼 적으면 80칸이면 보인다던 열이 실제 80칸에서 안 보인다.
...
- **제목** — `부동산 · 종목 3개 · Enter 로 전체`. 이 줄에서 무엇을 보고 있는지를 말하는
  건 섹터 이름 하나뿐이라 그것만 강조하고 나머지는 죽인다. 미실현·포텐셜은 여기서
...
화면이다. 창이 넷인데 셋만 섞이는 것은 누가 정한 규칙이 아니라 산술의 결과다. 사슬은
이렇다.
...
(`sector_numbers.py:381-383`) 5일이 저절로 빠진다. 5일을 빼기로 결정한 사람은 없다.
2차 차분이 5일치 데이터를 못 받을 뿐이다.
...
가 되어 널(49~52%)과 구분되지 않는 조합이 생긴다. 널보다 드물다는 판정 자체가 그
설정에서는 유지되지 않는다. 예전 README·화면이 적던 `17~34%` 는 잰 적 없는 좁은
범위였다(근거 스크립트의 260일 값을 화면의 범위인 양 옮겨 적은 것이다). 실측 주장은
잰 범위·잰 조건과 같이 적어야 한다. 그래서 화면은 숫자를 외우게 하지 않고 고른
주체·구간에 대해 상단 판정줄이 그때그때 답한다.
...
- **기각된 것은 한 가지 형태다 — "수급은 쓸모없다"가 아니다.**
  ...
  횡단면 롱온리 포트폴리오(손절 8%, 거래대금 ≥50억)"* 하나다. 판정문 자신이
  ...
  이미 고른 종목 안에서의 확인은 검정된 적이 없다. 기각이 아니라
...
이고, `kq-pead` 는 게이트를 통과한 유일한 알파의 재현용 백테스트(DB 읽기 전용)다. 이
저장소가 관측·검증 도구라 그 밖의 엔트리포인트는 없다.
