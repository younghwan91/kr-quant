...
def comove_lines(mo: Model, width: int) -> tuple[list[str], list[tuple], int]:
    ...
    50% 다. 개인·기관·외국인은 **2~38%**(시장 3 × 구간 4 = 12조합 전수, 2026-08-28)
    라 섹터들이 서로 **반대가 아니라 같이** 움직인다. **기타법인은 41~52% 로 널과
    구분되지 않는다** — 주체마다 다르므로 범위 하나로 단정하지 말고 헤더의 판정줄을
    읽어야 한다.
...
LIMITS = [
    ...
    "   범위는 시장 3 × 구간 4 = **12조합 전수**를 잰 값이다(2026-08-28). 예전엔",
    "   17~34% 라 적혀 있었는데, 그건 잰 적 없는 좁은 범위였다 — 실측 주장은",
    "   **잰 범위와 같이** 적어야 한다.",
...
LEDGER_HELP = [
    ...
    ("", "           범위는 시장 3 × 구간 4 = 12조합 전수다(2026-08-28) — 실측 주장은"),
    ("", "           **잰 범위와 같이** 적어야 한다. 예전의 17~34% 는 그러지 않았다."),
