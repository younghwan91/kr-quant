...
BANNER = ("관측: 주체×섹터 순매수.  미관측: 섹터→섹터 이동 · "
          "주체→주체 이전(주변합만 있다).")
...
def comove_lines(mo: Model, width: int) -> tuple[list[str], list[tuple], int]:
    ...
    50% 다. 실측은 17~34% 였다. 즉 섹터들은 서로 **반대가 아니라 같이** 움직인다.
    ...
    if obs is None:
        ...
        return [pad(note, width), pad(" " + BANNER, width)], [], 1
    ...
    l2 = " 같은/반대 방향으로 **동시에** 갔다는 사실이다. 옮겨갔다는 뜻이 아니다."
    if mo.detrend:
...
def limits_lines(width: int, top: int, height: int) -> tuple[list[str], int]:
    body = [pad(t, width) for t in LIMITS]
...
def screen(mo: Model, width: int, height: int) -> dict:
    ...
    out = body + [status_line(mo, width), pad(" " + BANNER, width)]
