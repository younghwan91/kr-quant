...
def comove_lines(mo: Model, width: int) -> tuple[list[str], list[tuple], int]:
    ...
    50% 다. 개인·기관·외국인은 **3~42%**(β제거 끈 상태, 시장 3 × 구간 4 = 12조합
    전수, 2026-09-01) 라 섹터들이 서로 **반대가 아니라 같이** 움직인다.
    **기타법인은 42~52% 로 널과 구분되지 않는다** — 그리고 ``d`` 로 β제거를 켜면
    셋도 17~54% 라 판정이 뒤집히는 조합이 있다. 주체마다·설정마다 다르므로 범위
...
LIMITS = [
    ...
    "   널을 못 이긴다 — 개인·기관·외국인은 음의 상관 쌍이 관측 **3~42%** 로",
    "   널(50%)보다 **드물다**. 섹터 자금흐름은 공통요인(위험선호)이 지배한다.",
    "   범위는 β제거 끈 상태로 시장 3 × 구간 4 = **12조합 전수**를 잰 값이다(2026-09-01).",
    "   β제거(d)를 켜면 셋도 17~54% 라 판정이 뒤집히는 조합이 있다.",
    "   예전엔 17~34% 라 적혀 있었는데 그건 잰 적 없는 좁은 범위였다 —",
    "   실측 주장은 **잰 범위와 같이** 적어야 한다.",
    "   **기타법인은 예외다** — 42~52% 라 구간을 어떻게 잡아도 널과 구분되지 않는다.",
...
LEDGER_HELP = [
    ...
    ("", "           **3~42%** 로 널보다 낮다(섹터는 반대가 아니라 **같이** 움직인다)."),
    ("", "           범위는 β제거 끈 상태로 12조합 전수다(시장 3 × 구간 4, 09-01)."),
    ("", "           β제거(d)를 켜면 셋도 17~54% 라 판정이 뒤집히는 조합이 있다."),
    ("", "           실측 주장은 **잰 범위와 같이** 적어야 한다 — 예전의 17~34% 는"),
    ("", "           그러지 않았다(근거 스크립트의 260일 값을 화면의 범위로 적었다)."),
    ("", "           기타법인은 42~52% 로 **널과 구분되지 않는다.** 판정줄이 화면에서"),
