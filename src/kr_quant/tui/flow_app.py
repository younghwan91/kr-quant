...
from kr_quant.tui.flow_view import (
    ...
    detail_lines, footer_line, header_lines, help_lines, name_sort_span,
    names_lines, sort_span, table_lines)
...
def hint_text(st: State, width: int = 200) -> str:
    ...
    return f" 정렬 {header}{arrow}{tag} · " + (desc or "? 로 설명")
...
def main() -> None:
    ...
    if term in ("", "dumb"):
        # dumb 터미널에서는 curses 가 커서를 못 옮긴다 — 예전엔 **빈 화면으로
        # 살아 있으면서** 키만 먹었다. 아무 말 없이 아무것도 안 하느니 나간다.
        raise SystemExit(
